...
class TrainingTab(QWidget):
    def __init__(self):
        ...
        self.text_name_input.setPlaceholderText("Name for this (optional)")
    ...
    def add_text(self):
        ...
        if not text:
            return
        name = self.text_name_input.text().strip() or "manual text"
        try:
            api_client.add_text_knowledge(text, name)
    ...
    def add_url(self):
        ...
        if not url:
            return
        name = self.url_name_input.text().strip() or None
        try:
            api_client.add_url_knowledge(url, name)
        except Exception as e:
            QMessageBox.warning(self, "Error", str(e))
        self.url_input.clear()
        self.url_name_input.clear()
        self.refresh()
    ...
    def add_pdf(self):
        ...
        if not path:
            return
        name = self.pdf_name_input.text().strip() or None
        try:
            api_client.add_pdf_knowledge(path, name)
        except Exception as e:
            QMessageBox.warning(self, "Error", str(e))
...
class ChatsTab(QWidget):
    def __init__(self):
        super().__init__()
        layout = QVBoxLayout(self)
        layout.addWidget(QLabel("Saved chats (also usable via /chats, /load, /delete in the terminal):"))
        self.list_widget = QListWidget()
        refresh_btn = QPushButton("Refresh")
        refresh_btn.clicked.connect(self.refresh)
        delete_btn = QPushButton("Delete Selected")
    ...
    def refresh(self):
        try:
            ...
            for c in api_client.list_chats():
                item = QListWidgetItem(f"{c['name']}  ({c['id'][:8]})")
...
class SettingsTab(QWidget):
    ...
    def on_ptt_done(self, heard: str, reply: str):
        ...
        self.ptt_result.setText(f"You: {heard}\nNOX: {reply}" if heard else reply)


class MainWindow(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("NOX — Chats, Training, Memory & Settings")
        self.resize(750, 650)

        tabs = QTabWidget()
        tabs.addTab(ChatsTab(), "Chats")
